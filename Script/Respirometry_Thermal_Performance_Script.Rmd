...
```{r}
lm.data<-snail.afdw.data %>% 
  dplyr::select(Snail_ID, Initial_Blotted_Wet_Mass_g, Final_Blotted_Wet_Mass_g, Ash_Free_Dry_Weight) %>% 
  filter(!is.na(Ash_Free_Dry_Weight)) %>%
  filter(!grepl("Control", Snail_ID))

#running linear regression for snail measurements (Final_Blotted_Wet_Mass_g versus Ash_Free_Dry_Weight)
lm.snail.data <- lm(Final_Blotted_Wet_Mass_g ~ Ash_Free_Dry_Weight, data = lm.data)
#running linear regression for snail measurements (Initial_Blotted_Wet_Mass_g versus Ash_Free_Dry_Weight)
lm.snail.data2 <- lm(Initial_Blotted_Wet_Mass_g ~ Ash_Free_Dry_Weight, data = lm.data)

# Extracting R-squared value
rsquared_final <- summary(lm.snail.data)$r.squared

# Plotting the data points
plot(lm.data$Ash_Free_Dry_Weight, lm.data$Final_Blotted_Wet_Mass_g, 
     xlab = "Ash Free Dry Weight", ylab = "Final Blotted Wet Mass (g)",
     main = "Linear Regression of Snail Measurements (Final Blotted Wet Mass vs. Ash Free Dry Weight)")

# Adding the regression line
abline(lm.snail.data, col = "red")

# Adding legend for regression line
legend("topleft", legend = paste("Regression Line (R^2 =", round(rsquared_final, 3), ")", sep = ""), col = "red", lty = 1)

# Adding text for R-squared value
text(x = min(lm.data$Ash_Free_Dry_Weight), y = max(lm.data$Final_Blotted_Wet_Mass_g), 
     labels = paste("R^2 =", round(rsquared_final, 3)), pos = 2)

# Running linear regression
lm.snail.data2 <- lm(Initial_Blotted_Wet_Mass_g ~ Ash_Free_Dry_Weight, data = lm.data)

# Extracting R-squared value
rsquared <- summary(lm.snail.data2)$r.squared

# Plotting the data points
plot(lm.data$Ash_Free_Dry_Weight, lm.data$Initial_Blotted_Wet_Mass_g, 
...
# Adding the regression line
abline(lm.snail.data2, col = "blue")

# Adding legend for regression line
legend("topleft", legend = paste("Regression Line (R^2 =", round(rsquared, 3), ")", sep = ""), col = "blue", lty = 1)

# Adding text for R-squared value
text(x = min(lm.data$Ash_Free_Dry_Weight), y = max(lm.data$Initial_Blotted_Wet_Mass_g), 
