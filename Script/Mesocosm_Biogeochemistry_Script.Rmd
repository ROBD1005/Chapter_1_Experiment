...
mesocosm.data <- read_csv(here::here("Data", "Mesocosm_Data", "Completed_Mesocosm_Dataset_Carbonate.csv"))
mesocosm.data <- mesocosm.data %>%
  mutate(Datetime = ymd_hms(paste0(Date, "10:00:00"))) %>%
  select(Datetime, Tank_ID, Temp_Treatment, pH_Treatment, Temp_C_Spot=Temp_C) %>%
  na.omit()

# join the time series and the spot check data 
filtered_timeseries <- filtered_timeseries %>%
  left_join(mesocosm.data, by = c("Datetime", "Tank_ID", "Temp_Treatment", "pH_Treatment")) %>% 
  mutate(Date = as.Date(Datetime))

# 1. Calculate the difference between spot check and logger value at 10 AM
...
means <- tank_timeseries_corrected %>%
  filter(pH_Treatment != "Sump", Temp_Treatment != "10") %>%
  group_by(Temp_Treatment, pH_Treatment) %>%
