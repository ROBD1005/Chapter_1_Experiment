...
# Function to render LaTeX equations to images
render_latex_to_image <- function(latex, file_path) {
  ...
  system(paste("pdflatex -output-directory", dirname(tex_file), tex_file), ignore.stdout = TRUE)
  
  # Convert the .pdf to .png
  img <- image_read(pdf_file)
  image_write(img, path = file_path, format = "png")
  
  # Clean up temporary files
  file.remove(tex_file, pdf_file, sub("\\.tex$", ".aux", tex_file), sub("\\.tex$", ".log", tex_file))
}

# Define LaTeX equations and file paths
equations <- list(
  list(latex = "\\text{CO}_2(g) \\leftrightharpoons \\text{CO}_2(aq) \\quad (K_H)", file = here("Figures", "equation1.png")),
  list(latex = "\\text{H}_2\\text{CO}_3 \\leftrightharpoons \\text{H}^+ + \\text{HCO}_3^- \\quad (K_1)", file = here("Figures", "equation2.png")),
  list(latex = "\\text{HCO}_3^- \\leftrightharpoons \\text{H}^+ + \\text{CO}_3^{2-} \\quad (K_2)", file = here("Figures", "equation3.png")),
  list(latex = "\\text{H}_2\\text{O} \\leftrightharpoons \\text{H}^+ + \\text{OH}^- \\quad (K_w)", file = here("Figures", "equation4.png")),
  list(latex = "\\text{TA} = [\\text{HCO}_3^-] + 2[\\text{CO}_3^{2-}] + [\\text{B(OH)}_4^-] + [\\text{OH}^-] - [\\text{H}^+]", file = here("Figures", "equation5.png"))
)

# Render and save each equation as an image
for (eq in equations) {
  render_latex_to_image(eq$latex, eq$file)
}

# Output markdown with images
cat("
## Equilibria and Constants Used in Calculations

The carbonate system parameters calculated in this study are governed by the following equilibria and constants:

1. **Gas Dissolution in Water**:
![Equation 1](Figures/equation1.png)

2. **First Dissociation of Carbonic Acid**:
![Equation 2](Figures/equation2.png)

3. **Second Dissociation of Carbonic Acid**:
![Equation 3](Figures/equation3.png)

4. **Water Dissociation**:
![Equation 4](Figures/equation4.png)

5. **Titration Alkalinity (TA)**:
![Equation 5](Figures/equation5.png)

## References

Dickson, A. G. (1981). The Carbon Dioxide System in Seawater: Equilibrium Chemistry and Measurements. *Marine Chemistry*, 10(2), 197-217.

Johansson, O., & Wedborg, M. (1982). The Buffer Capacity of Sea Water. *Marine Chemistry*, 11(4), 315-322.

Park, K. (1969). The Carbon Dioxide System in Seawater: Equilibrium Chemistry. *Limnology and Oceanography*, 14(1), 12-21.

Skirrow, G. (1965). The Dissociation Constants of Carbonic Acid in Seawater. *Marine Chemistry*, 2, 202-205.

Skirrow, G. (1975). The Solubility of Gases in Water and Seawater. *Marine Chemistry*, 5, 111-130.

## Software Reference

The carbonate system calculations were performed using the \\texttt{seacarb} package in R. The \\texttt{seacarb} package provides functions for calculating parameters of the seawater carbonate system and is based on the formulations recommended by Dickson et al. (2007).

Lavigne, H., Orr, J., & Gattuso, J.-P. (2011). *seacarb: Seawater Carbonate Chemistry with R*. [R package version 3.3.0](https://CRAN.R-project.org/package=seacarb).
")
...
# Check the class of Temp_Treatment
str(joule.rates.dataset$Temp_Treatment)

# Check the class of the entire dataset
class(joule.rates.dataset)
```
