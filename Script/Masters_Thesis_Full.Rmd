...
TPC_schematic <- ggplot() +
  ...
  geom_text(aes(x =  high_ctmax-3, y = high_rmax[1]*0.4), label = TeX("$CT_{max}$"), size=5, color = "black", family = "serif") +  # Label for CTmax 
  ...
  geom_segment(aes(x = 1, y = high_rmax[1]*0.35, xend = 1, yend = high_rmax[1]*0.225), 
               arrow = arrow(type = "closed", length = unit(0.1, "inches")), color = "black", size = 0.5) +
  geom_text(aes(x = 1, y = high_rmax[1]*0.4), label = TeX("$CT_{min}$"), size=5, color = "black", family = "serif") +  # Label for CTmin
  ...
  geom_text(aes(x = 1, y = high_rmax[1]), label = TeX("$R_{max}$"), size=5, vjust = -0.5, color = "black", family = "serif") +
  geom_text(aes(x = high_topt[1], y = high_rmax[1]), label = TeX("$T_{opt}$"), size=5, vjust = -0.5, color = "black", family = "serif") +  # Label for Thermal Optimum
  ...
  geom_text(aes(x = middle_point_high[1], y = high_rmax[1]* 0.8), label = TeX("$T_{br}$"), size=5, vjust = 1.3, hjust=0.8, color = "black", family = "serif") +
  ...
  geom_text(aes(x = high_topt-5, y = high_rmax[1] * 0.925), label = TeX("$E_{a}$"), size=5, color = "black", family = "serif") +
  geom_segment(aes(x = high_topt+3, xend = high_topt+5, y = high_rmax[1]*0.88, yend = high_rmax[1]*0.6), 
                 arrow = arrow(length = unit(0.3, "cm")), color = "black") +
  geom_text(aes(x = high_topt+4.5, y = high_rmax[1] * 0.825), label = TeX("$E_{h}$"), size=5, color = "black", family = "serif") +
...
scale_fill_distiller(palette = "YlOrRd", limits = c(0, 26), guide = guide_colorbar(title = bquote("Temperature ("*degree*C*")"), title.position = "bottom", ticks.colour = NA), direction=1) +
...
theme_minimal() +  # Start with a minimal theme
  ...
  theme(plot.background = element_blank(),  # Remove background elements
        ...
        legend.title = element_text(hjust = 0.5, size = rel(1.5), family="serif"),  # Apply serif font to legend title
  ...
  annotate("text", x = square_data$xmax+3.1, y = square_data$ymax-3.75, family = "serif", label = "Ambient pH", size=4.5, color = "black") +
  annotate("text", x = square_data$xmax+3.65, y = square_data$ymax-11.75, family = "serif", label = "Decreased pH", size=4.5, color = "black")

print(TPC_schematic)

ggsave("TPC_schematic.png", plot = TPC_schematic, width = 8, height = 6, dpi = 1300)
...
common_theme <- theme_minimal(base_family="serif") +
  theme(
    ...
    strip.text = element_text(hjust = 0.5, size = 12, family = "serif"), 
    strip.text.x = element_text(vjust = 0.5, size = 12, family = "serif"), 
    strip.text.y = element_text(vjust = 0.5, size = 12, family = "serif"), 
    axis.title.x = element_text(margin = margin(t = 10), size = 14, hjust = 0.5, family = "serif"),
    axis.title.y = element_text(margin = margin(r = 10), size = 14, vjust = 0.5, family = "serif"),  # Same size for both axes
    axis.text = element_text(size = 10, family = "serif"),  # Same size for both axes
...
heat_colors <- c(
    ...
    "#fc4e2a",  # June
    "#e31a1c",  # July
    "red3",  # August - peak at darkest red
    "#e31a1c",  # September
    "#fc4e2a",  # October
    "#ff9a17",  # November
    "#ffd480"   # December - back to light yellow
)
...
# Define the plot
temp_violin_plot <- ggplot(monthly_temp_stats, aes(x = Month, y = Mean_Temp, fill= Month, color = Month)) +
  geom_hline(yintercept = seq(12, 26, by = 2), color = "darkorange1", size = 0.5) +
  geom_violin(trim = FALSE, scale="width", color="black") +
  geom_boxplot(width = 0.3, outlier.shape = NA, color="black", fill="white", alpha=0.6) +  # Add boxplot inside violin
  scale_color_manual(values = heat_colors) +  # Use the custom color vector
  scale_fill_manual(values = heat_colors) +
  common_theme+
  theme(plot.title = element_blank(), plot.subtitle = element_blank(), legend.position = "none", axis.text = element_text(size = 12)) +
