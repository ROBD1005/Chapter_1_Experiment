...
# Setting up the R environment, loading essential packages, and defining custom functions
...
install_if_missing <- function(package_name) {
  if (!package_name %in% rownames(installed.packages())) {
    tryCatch({
      ...
      message(sprintf("Package '%s' successfully installed.", package_name))
    }, error = function(e) {
      cat("Error installing package:", package_name, "- Error message:", e$message, "\n")
  ...
  tryCatch({
    ...
    cat("Error loading package:", package_name, "- Error message:", e$message, "\n")
  })
}

# Install and load essential packages
essential_packages <- c("remotes", "devtools", "pacman", "utils", "here", "knitr", "dplyr", "tools", "xfun")
invisible(lapply(essential_packages, install_if_missing))

# List of packages with GitHub repositories
specific_packages <- list(
  pacman = "trinker/pacman",
  downloadthis = "fmmattioni/downloadthis",
  ggmapinset = "cidm-ph/ggmapinset",
  ggpubr = "kassambara/ggpubr",
  gt = "rstudio/gt",
  gtsummary = "ddsjoberg/gtsummary",
  gtExtras = "jthomasmock/gtExtras",
  rnaturalearthhires = "ropensci/rnaturalearthhires",
  rTPC = "padpadpadpad/rTPC",
  LoLinR = "colin-olito/LoLinR",
  nls.multstart = "padpadpadpad/nls.multstart",
  ggchicklet = "hrbrmstr/ggchicklet"
)

# Loop through the list to install packages that are not already loaded
for (pkg in names(specific_packages)) {
    if (!require(pkg, character.only = TRUE)) {
        tryCatch({
            remotes::install_github(specific_packages[[pkg]], upgrade = "always", force = TRUE, dependencies = TRUE, quiet = TRUE)
            library(pkg, character.only = TRUE)
            message(sprintf("Package '%s' successfully installed and loaded using remotes.", pkg))
        }, error = function(e) {
            cat("Error installing/loading package using remotes:", pkg, "- Error message:", e$message, "\n")
            tryCatch({
                devtools::install_github(specific_packages[[pkg]], upgrade = "always",
                                         force = TRUE, dependencies = TRUE, quiet = TRUE)
                library(pkg, character.only = TRUE)
                message(sprintf("Package '%s' successfully installed and loaded using devtools.", pkg))
            }, error = function(e) {
                cat("Error installing/loading package using devtools:", pkg, "- Error message:", e$message, "\n")
            })
        })
    } else {
        message(sprintf("Package '%s' is already installed and loaded.", pkg))
    }
}


# Function to load, install, and verify packages
load_and_verify_packages <- function(packages) {
  # Load and install the packages using pacman
  pacman::p_load(char = packages, character.only = TRUE, install = TRUE, update = TRUE)
  
  # Verify that the packages are loaded
  loaded_packages <- sapply(packages, require, character.only = TRUE)
  
  # Display the loading status of each package
  print(loaded_packages)
}

# General packages
general_packages <- c(
  "base", "broom", "car", "carData", "datasets", "deps", "devtools", "downloadthis", 
  "forcats", "dplyr", "glue", "here", "htmltools", "knitr", "latex2exp", "pacman", "readr", 
  "remotes", "rmarkdown", "stringr", "tibble", "tidyr", "tidyverse", "tools", "usethis", "utils"
)
load_and_verify_packages(general_packages)

# Plotting and visualization
plotting_packages <- c(
  "cowplot", "extrafont", "ggchicklet", "ggforce", "ggfx", "ggimage", "gginnards", 
  "ggmap", "ggmapinset", "ggplot2", "ggpmisc", "ggpp", "ggpubr", "ggrepel", "ggsci", 
  "ggspatial", "ggstatsplot", "ggsignif", "ggthemes", "grid", "gridExtra",
  "gt", "gtExtras", "gtsummary", "hrbrthemes", "latex2exp", "magick", "markdown",
  "patchwork", "png", "RColorBrewer", "scales", "tibble"
)
load_and_verify_packages(plotting_packages)

# Geospatial data handling
geospatial_packages <- c(
  "ggmap", "ggmapinset", "ggspatial", "lwgeom", "raster", "rnaturalearth", 
  "rnaturalearthdata", "rnaturalearthhires", "sf", "sp"
)
load_and_verify_packages(geospatial_packages)

# Statistical analysis and modeling
statistical_packages <- c(
  "AICcmodavg", "boot", "lme4", "lmerTest", "LoLinR", "MASS", "Matrix", "methods", "minpack.lm", "multcomp",
  "MuMIn", "nlstools", "nls.multstart", "NISTunits", "performance", "rstatix", 
  "rTPC", "respR", "respirometry", "seacarb", "stats", "zoo"
)
load_and_verify_packages(statistical_packages)

# Citations
citation_packages <- c(
  "grateful", "renv", "revtools"
)
load_and_verify_packages(citation_packages)
...
# Bibliography
pkgs <- grateful::cite_packages(pkgs = "All", include.RStudio = TRUE, cite.tidyverse = TRUE, 
                        output = "table", out.dir = here::here("Citations"), 
                        bib.file = "thesis-citations.bib")
...
## Equations for Sewater Carbonate System Constraints and Constants

```{r}


# Define the LaTeX equation
latex_equation <- TeX("$$y = \\frac{(x - \\mu)}{(\\text{max} - \\text{min})}$$")

# Conditionally display the LaTeX equation
if (knitr::is_html_output()) {
  # For HTML output
  div(style = "text-align: center;", latex_equation)
} else if (knitr::is_latex_output()) {
  # For LaTeX output
  knitr::asis_output(latex_equation)
}

# References:
# K*SO4: Khoo et al. (1977), Dickson (1990a), or Waters and Millero (2013)
# K*HF: Dickson and Riley (1979) or Perez and Fraga (1987)
# K*NH3: Clegg and Whitfield (1995)
# K*H2S: Yao and Millero (1995)
# H+ activity coefficient: Takahashi et al. (1982)
# Humidity correction: Weiss and Price (1980)
...
gt_meso_summary <- mesocosm.data %>%
  ...
  mutate(
    ...
    Temp_Treatment = factor(paste0(Temp_Treatment, " ºC"),
    levels = c("12 ºC", "14 ºC", "16 ºC", "18 ºC", "20 ºC", "22 ºC", "24 ºC", "26 ºC")),
  ...
  for (variable in names(variable_labels)) {
    ...
    for (temp in temp_treatments) {
      filtered_data <- gt_meso_summary %>%
        filter(Temp_Treatment == paste0(temp, " ºC")) %>%
        ...
        modify_table_body(
          ~ .x %>%
            mutate(
              label = ifelse(
                row_type == "label",
                gt::html(sprintf("<b>Temperature&nbsp;%sºC<b/>", temp)),
                label
              )
            )
        ) %>%
...
variable_labels <- list(
  ...
  "TA" = 
    "<span style='font-weight: bold;'>TA</span> <br/>
     <span class='unit' style='font-size: 90%;font-weight: bold;'>(µmol&nbsp;kg<sup>−1</sup>)</span>",
  "pCO2insitu" = 
    "<span style='font-weight: bold;'>pCO<sub>2</sub></span> <br/>
     <span class='unit' style='font-size: 90%;font-weight: bold;'>(µatm)</span>",
  "HCO3" =
...
mesocosm.data.filtered <- mesocosm.data %>%
  ...
  mutate(
    Temp_Treatment = factor(paste0(Temp_Treatment, " ºC"),
                            levels = c("12 ºC", "14 ºC", "16 ºC", "18 ºC",
                                       "20 ºC", "22 ºC", "24 ºC", "26 ºC")),
...
variable_labels <- list(
  ...
  TA = "TA (µmol kg<sup>−1</sup>)",
  pCO2insitu = "pCO<sub>2</sub> (µatm)",
  HCO3 = "HCO<sub>3</sub><sup>-</sup> (µmol kg<sup>−1</sup>)",
  CO3 = "CO<sub>3</sub><sup>2-</sup> (µmol kg<sup>−1</sup>)",
  DIC = "DIC (µmol kg<sup>−1</sup>)"
)
...
generate_summary_tables <- function(gt_meso_summary, variable_labels) {
  ...
  for (temp in temp_treatments) {
    temp_table <- gt_meso_summary %>%
      ...
      modify_header(
        label ~ "<b>Variable (x̅ ± σ<sub>M</sub>)</b>",
  ...
  tab_spanners <- sprintf("<b style='white-space: nowrap;'>Temperature %s ºC</b>",
                          temp_treatments)
...
normality_tests <- joule.rates.dataset %>%
  group_by(pH_Treatment, Temp_Treatment) %>%
  summarise(p_value = shapiro.test(J.gram.hr)$p.value)
...
levene_test <- leveneTest(J.gram.hr ~ interaction(pH_Treatment, Temp_Treatment), data = joule.rates.dataset)
...
joule.rates.dataset <- respo.rates.dataset %>%
  ...
  mutate(
    ...
    dplyr::select(Snail_ID, Temp_Treatment, pH_Treatment, Temp.C, J.gram.hr)
```
